...
function howManyMovies(moviesArray) {
    return moviesArray.filter(movie => movie.director ==='Steven Spielberg').map(movie=>movie.genre === 'Drama').length;

}

// Iteration 3: All scores average - Get the average of all scores with 2 decimals
function scoresAverage(moviesArray) {
    return moviesArray.filter(movie => movie.score).reduce((accum, curr) =>{ 
        return ( accum.score + curr).toFixed(2)}, 0) / moviesArray.length;
         
}

// Iteration 4: Drama movies - Get the average of Drama Movies
function dramaMoviesScore(moviesArray) {
    return moviesArray.filter(movie => movie.genre === 'Drama').reduce((accum, curr) =>{ 
        return ( accum.score + curr).toFixed(2)}, 0) / moviesArray.length;
